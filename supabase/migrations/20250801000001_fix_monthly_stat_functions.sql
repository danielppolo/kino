...
-- Create fixed function to update monthly category stats
...
BEGIN
    ...
    IF NEW.category_id IS NOT NULL THEN
        -- For UPDATE operations within the same category and month, we need to handle differently
        IF TG_OP = 'UPDATE' AND 
           OLD.category_id = NEW.category_id AND 
           date_trunc('month', OLD.date) = date_trunc('month', NEW.date) AND
           OLD.wallet_id = NEW.wallet_id THEN
            -- Update existing record by replacing the old values with new ones
            UPDATE monthly_category_stats
            SET
                income_cents = income_cents - 
                    CASE WHEN OLD.type = 'income' THEN OLD.amount_cents ELSE 0 END +
        ...
        ELSE
            -- For INSERT operations or when category/month/wallet changes, use INSERT with ON CONFLICT
...
-- Create fixed function to update monthly label stats
...
BEGIN
    ...
    IF NEW.label_id IS NOT NULL THEN
        -- For UPDATE operations within the same label and month, we need to handle differently
        IF TG_OP = 'UPDATE' AND 
           OLD.label_id = NEW.label_id AND 
           date_trunc('month', OLD.date) = date_trunc('month', NEW.date) AND
           OLD.wallet_id = NEW.wallet_id THEN
            -- Update existing record by replacing the old values with new ones
            UPDATE monthly_label_stats
            ...
            WHERE wallet_id = NEW.wallet_id 
                ...
                AND month = date_trunc('month', NEW.date);
        ELSE
            -- For INSERT operations or when label/month/wallet changes, use INSERT with ON CONFLICT
