-- Function to backfill historical monthly stats including transfers
CREATE OR REPLACE FUNCTION backfill_monthly_stats_with_transfers()
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  FOR wallet_record IN SELECT id FROM wallets LOOP
    ...
    WHILE month_start <= month_end LOOP
      -- Calculate income (positive amounts) for this month
      ...
      -- Calculate outcome (negative amounts) for this month
      SELECT COALESCE(SUM(ABS(amount_cents)), 0) INTO outcome_cents
      FROM transactions
      WHERE wallet_id = wallet_record.id
        AND amount_cents < 0
        AND date >= month_start
...
SELECT backfill_monthly_stats_with_transfers();
