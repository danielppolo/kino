...
export function TagCloudAnalyticsChart({
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="flex flex-wrap gap-4 items-center justify-center min-h-[200px] py-4">
          {tagData.slice(0, 20).map((tag, index) => (
            <div
              key={tag.tag}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-muted hover:bg-muted/80 transition-colors cursor-pointer"
              style={{
        ...
        <div className="mt-4 pt-4 border-t">
          <h4 className="text-sm font-medium mb-2">Top Tags by Amount</h4>
          <div className="grid gap-2">
            {tagData.slice(0, 5).map((tag) => (
              <div key={tag.tag} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{tag.tag}</span>
                  <span className="text-muted-foreground">({tag.count} tx)</span>
                </div>
