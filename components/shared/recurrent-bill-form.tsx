...
interface RecurrentBillFormProps {
  onSuccess?: () => void;
  open?: boolean;
...
interface RecurrentBillFormValues {
  wallet_id: string;
  wallet_ids: string[];
  description: string;
  amount: string;
...
export function RecurrentBillForm({
  onSuccess,
  open,
  onOpenChange,
  recurrentBill,
  defaultWalletId,
}: RecurrentBillFormProps) {
  ...
  const form = useForm<RecurrentBillFormValues>({
    defaultValues: {
      wallet_id:
  ...
  const createMutation = useMutation({
    mutationFn: async (values: RecurrentBillFormValues) => {
      const walletIds =
  ...
  const onSubmit = (values: RecurrentBillFormValues) => {
    if (isEdit) {
...
export default RecurrentBillForm;
