...
```mermaid
xychart-beta
  title "Nest egg needed (MXN) vs annual spending, by withdrawal rate"
  x-axis ["180k","240k","360k","480k","600k"]
  y-axis "Nest egg (MXN millions)" 0 --> 32
  line "2%" [9,12,18,24,30]
  line "3%" [6,8,12,16,20]
  line "3.5%" [5.14,6.86,10.29,13.71,17.14]
  line "4%" [4.5,6,9,12,15]
  line "5%" [3.6,4.8,7.2,9.6,12]
```
...
```mermaid
flowchart TD
  A[Define non-housing annual spend S] --> B[Choose target WR range 3.0%–3.5% base]
  B --> C[Compute FIRE number = S / WR]
  C --> D[Build 6–24 months liquidity layer]
  D --> E[Build inflation-defense layer]
  E --> F[Build growth layer with global diversification]
  F --> G[Select withdrawal policy: guardrails + rebalancing]
  G --> H[Stress tests: inflation, drawdown, FX, healthcare]
  H --> I[Adjust: spend, WR, or add part-time income]
```
...
```mermaid
xychart-beta
  title "Approx. years to FIRE vs savings rate (WR=3.5%, real return=4%)"
  x-axis ["20%","30%","40%","50%","60%","70%"]
  y-axis "Years" 0 --> 50
  bar "Years to FI" [44,33,26,19,15,10]
```
